...
ZIPNAME="CrDroid-NetHunter-Loukious-$(git describe --abbrev=0 --tags)-$(date '+%Y%m%d-%H%M').zip"
export PROC="-j$(nproc)"
...
# Compress the Image file to create Image.gz
if [ -f "$OUT/arch/arm64/boot/Image" ]; then
    gzip -k "$OUT/arch/arm64/boot/Image"
    echo "Image.gz created successfully."
else
    echo "Error: Image file not found in $OUT/arch/arm64/boot/."
    exit 1
fi
